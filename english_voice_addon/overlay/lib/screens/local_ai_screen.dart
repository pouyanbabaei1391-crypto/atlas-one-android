      ...
      body: AnimatedBuilder(
        ...
        builder: (context, _) {
          ...
          final labels = {'not_installed': 'Model not installed', 'installing': 'Installing model…',
            ...
            'loading': 'Loading and warming Gemma…', 'ready': 'Ready on this phone', 'generating': 'Generating locally…'};
          return ListView(padding: const EdgeInsets.all(20), children: [
            ...
            if (!model.ready && !model.preparing) ...[
              CheckboxListTile(value: accepted, onChanged: (value) => setState(() => accepted = value ?? false),
                contentPadding: EdgeInsets.zero, title: const Text('I agree to the Gemma Terms of Use and Prohibited Use Policy.')),
              TextButton(onPressed: () => launchUrl(Uri.parse('https://ai.google.dev/gemma/terms')), child: const Text('Read Gemma terms')),
              TextButton(onPressed: () => launchUrl(Uri.parse('https://ai.google.dev/gemma/prohibited_use_policy')), child: const Text('Read use policy')),
              FilledButton.icon(onPressed: !accepted ? null : () async {
                await c.settings.acceptModelTerms();
                await c.prepareLocalAi();
              }, icon: const Icon(Icons.download_rounded),
                label: Text(model.installed ? 'Load Gemma on this phone' : 'Install and prepare Gemma')),
            ],
