...
void AtlasEngine::load(const std::string &path, int threads) {
    ...
    if (!model) throw std::runtime_error("Qwen3 could not load. Check free RAM and the model installation.");
    ...
    if (!context) { clear(); throw std::runtime_error("Not enough memory for the Qwen3 context."); }
...
void AtlasEngine::generate(const std::string &prompt, int limit, const std::function<void(const std::string&)> &emit) {
    if (!ready()) throw std::runtime_error("Install and load Qwen3 before starting voice chat.");
